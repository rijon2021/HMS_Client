...
@Component({
  selector: 'app-bc-case-inspection-list',
  templateUrl: './bc-case-inspection-list.component.html',
  styleUrls: ['./bc-case-inspection-list.component.css']
})
export class BcCaseInspectionListComponent implements OnInit {
  ...
  getAll() {
    this.applicationFileMasterService.getListByType(ApplicationType.BCCase).subscribe(
      (res) => {
        if (res) {
  ...
  printModal(name: string) {
    if (this.selectedFile.applicationFileMasterID > 0) {
      ...
      // RoutingHelper.navigate2([], ['cda', 'rpt-bc-case-inspection-print', this.selectedFile.applicationFileMasterID], this.router);
    }
  }
