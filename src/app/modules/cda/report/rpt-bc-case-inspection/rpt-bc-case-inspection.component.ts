...
@Component({
  selector: 'app-rpt-bc-case-inspection',
  templateUrl: './rpt-bc-case-inspection.component.html',
  styleUrls: ['./rpt-bc-case-inspection.component.css']
})
export class RptBcCaseInspectionComponent implements OnInit {
  @ViewChild("modalBcCaseReport") modalBcCaseReport: TemplateRef<any>;

  public fileMasterID: number;
  public userRoleID: number;
  showEdit: boolean;
  pageTitle: string = "BC Case Building Inspection";
  reportName: string;
  ...
  getAll() {
    this.applicationFileMasterService.getListByType(ApplicationType.BCCase, InspectionFileType.Inspection, true).subscribe(
      (res) => {
        if (res) {
